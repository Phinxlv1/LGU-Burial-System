...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .topbar-sep { color: #d1d5db; font-size: 16px; }
        ...
        .stat-lbl { font-size: 10px; font-weight: 600; color: #9ca3af; text-transform: uppercase; letter-spacing: .06em; margin-top: 4px; }
        ...
        .panel-head-info h2 { font-size: 14px; font-weight: 700; color: #111827; }
        .panel-head-info p  { font-size: 12px; color: #9ca3af; margin-top: 2px; }

        .panel-body { padding: 1.25rem; display: flex; flex-direction: column; gap: .75rem; }
        ...
        .filter-btn:hover { border-color: #1a2744; color: #1a2744; }
        .filter-btn.f-all.active    { background: #1a2744; color: #fff; border-color: #1a2744; }
        .filter-btn.f-dup.active    { background: #ef4444; color: #fff; border-color: #ef4444; }
        .filter-btn.f-miss.active   { background: #f59e0b; color: #fff; border-color: #f59e0b; }
        .filter-btn.f-incon.active  { background: #3b82f6; color: #fff; border-color: #3b82f6; }
        ...
        .btn-rescan:hover { background: #f0f4ff; border-color: #1a2744; color: #1a2744; }
        ...
        .issue.sev-low    { border-left-color: #3b82f6; }
        ...
        .issue-head:hover        { background: #f3f4f6; }
        .issue.open .issue-head  { background: #f3f4f6; }
        ...
        .badge-incon { background: #dbeafe; color: #1e40af; }

        .issue-title { font-size: 13px; font-weight: 600; color: #111827; flex: 1; }
        .issue-count { font-size: 11px; color: #9ca3af; white-space: nowrap; }

        .chevron { color: #9ca3af; transition: transform .2s; flex-shrink: 0; }
        .issue.open .chevron { transform: rotate(90deg); }

        .issue-body { display: none; border-top: 1px solid #f3f4f6; }
        ...
        .rec-info   { flex: 1; min-width: 180px; }
        .rec-title  { font-size: 13px; font-weight: 500; color: #111827; }
        .rec-sub    { font-size: 11px; color: #9ca3af; margin-top: 1px; }
        ...
        .chip-miss  { background: #fef3c7; border: 1px solid #fde68a; color: #92400e; }
        .chip-bad   { background: #fee2e2; border: 1px solid #fca5a5; color: #991b1b; }
        .chip-info  { background: #dbeafe; border: 1px solid #93c5fd; color: #1e40af; }

        .rec-actions { display: flex; gap: .4rem; flex-shrink: 0; }
        ...
        .dq-empty h3 { font-size: 15px; font-weight: 700; color: #065f46; }
        .dq-empty p  { font-size: 12px; color: #9ca3af; margin-top: 4px; }
        ...
        .toast-icon.amber { background: #fef3c7; }
        ...
        .modal-header h3 { font-size: 14px; font-weight: 700; color: #fff; display:flex; align-items:center; gap:7px; }
        .modal-close { background: none; border: none; cursor: pointer; color: rgba(255,255,255,.6); font-size: 20px; line-height: 1; }
        .modal-close:hover { color: #fff; }
        .modal-body { padding: 1.25rem; display: flex; flex-direction: column; gap: 1rem; }
        .modal-footer { padding: .85rem 1.25rem; border-top: 1px solid #f3f4f6; display: flex; justify-content: space-between; gap: .6rem; }
        .btn-cancel { padding: .5rem 1rem; border-radius: 6px; border: 1px solid #e5e7eb; font-family: 'DM Sans', sans-serif; font-size: 13px; color: #374151; background: #fff; cursor: pointer; }
        .btn-cancel:hover { background: #f9fafb; }
        .btn-delete-confirm {
            display: inline-flex; align-items: center; gap: 6px;
            padding: .5rem 1.1rem; background: #dc2626; color: #fff;
        ...
        html.dark .stat-card.red    .stat-val { color: #f87171 !important; }
        html.dark .stat-card.amber  .stat-val { color: #fbbf24 !important; }
        html.dark .stat-card.blue   .stat-val { color: #60a5fa !important; }
        html.dark .stat-card.green  .stat-val { color: #34d399 !important; }
        ...
        html.dark .panel-head-info h2   { color: #e2e8f0 !important; }
        html.dark .panel-head-info p    { color: #64748b !important; }

        html.dark .filter-btn           { background: #252840 !important; border-color: #374151 !important; color: #94a3b8 !important; }
        html.dark .filter-btn:hover     { border-color: #6366f1 !important; color: #e2e8f0 !important; }
        html.dark .filter-btn.f-all.active   { background: #1a2744 !important; border-color: #1a2744 !important; color: #fff !important; }
        html.dark .filter-btn.f-dup.active   { background: #ef4444 !important; border-color: #ef4444 !important; color: #fff !important; }
        html.dark .filter-btn.f-miss.active  { background: #f59e0b !important; border-color: #f59e0b !important; color: #fff !important; }
        html.dark .filter-btn.f-incon.active { background: #3b82f6 !important; border-color: #3b82f6 !important; color: #fff !important; }

        html.dark .btn-rescan           { background: #252840 !important; border-color: #374151 !important; color: #94a3b8 !important; }
        html.dark .btn-rescan:hover     { background: #2d3148 !important; border-color: #6366f1 !important; color: #e2e8f0 !important; }

        html.dark .btn-sm               { background: #252840 !important; border-color: #374151 !important; color: #94a3b8 !important; }
        html.dark .btn-sm:hover         { background: #2d3148 !important; border-color: #6366f1 !important; color: #e2e8f0 !important; }
        html.dark .btn-sm.danger        { border-color: #7f1d1d !important; color: #f87171 !important; }
        html.dark .btn-sm.danger:hover  { background: #2a1a1a !important; border-color: #ef4444 !important; }
        html.dark .btn-sm.warn          { border-color: #78350f !important; color: #fbbf24 !important; }
        html.dark .btn-sm.warn:hover    { background: #1c1200 !important; border-color: #f59e0b !important; }

        html.dark .issue                { border-color: #2d3148 !important; }
        html.dark .issue-head           { background: #181b29 !important; }
        html.dark .issue-head:hover,
        html.dark .issue.open .issue-head { background: #252840 !important; }
        html.dark .issue-title          { color: #e2e8f0 !important; }
        html.dark .issue-count          { color: #64748b !important; }
        html.dark .issue-body           { border-top-color: #2d3148 !important; }
        html.dark .issue-desc           { background: #181b29 !important; color: #94a3b8 !important; border-bottom-color: #2d3148 !important; }

        html.dark .badge-dup            { background: #450a0a !important; color: #fca5a5 !important; }
        html.dark .badge-miss           { background: #422006 !important; color: #fde68a !important; }
        html.dark .badge-incon          { background: #1e3a5f !important; color: #93c5fd !important; }

        html.dark .record               { border-bottom-color: #1e2130 !important; }
        html.dark .rec-title            { color: #e2e8f0 !important; }
        html.dark .rec-sub              { color: #64748b !important; }

        html.dark .chip-miss            { background: #422006 !important; border-color: #78350f !important; color: #fde68a !important; }
        html.dark .chip-bad             { background: #450a0a !important; border-color: #7f1d1d !important; color: #fca5a5 !important; }
        html.dark .chip-info            { background: #1e3a5f !important; border-color: #1e40af !important; color: #93c5fd !important; }

        html.dark .spinner              { border-color: #2d3148 !important; border-top-color: #6366f1 !important; }
        html.dark .dq-empty-icon        { background: #052e16 !important; }
        html.dark .dq-empty h3          { color: #34d399 !important; }
        ...
        html.dark .dq-no-match          { color: #64748b !important; }

        @media print {
            .sa-sidebar, .topbar { display: none; }
            .main { margin-left: 0; }
        }
    </style>
</head>
            ...
            <div class="panel-head">
                ...
                <div class="toolbar">
                    <div class="filters">
                        ...
                        <button class="filter-btn f-dup"        onclick="setFilter('duplicate', this)">🔴 Duplicates</button>
                        <button class="filter-btn f-miss"       onclick="setFilter('missing', this)">🟡 Missing Data</button>
                        <button class="filter-btn f-incon"      onclick="setFilter('inconsistent', this)">🔵 Inconsistent</button>
                    </div>
...
<div class="modal-overlay" id="deleteModal" onclick="if(event.target===this)closeDeleteModal()">
    <div class="modal">
        ...
        <div class="modal-body">
            <div style="display:flex;align-items:center;gap:12px;padding:.75rem 1rem;background:#fff5f5;border:1px solid #fecaca;border-radius:8px">
                ...
                <div>
                    <div style="font-size:13px;font-weight:700;color:#991b1b">You are about to permanently delete:</div>
                    <div style="font-size:12px;color:#b91c1c;margin-top:3px">This action <strong>cannot be undone.</strong></div>
                </div>
            </div>
            <div style="display:flex;flex-direction:column;gap:6px">
                <div style="display:flex;align-items:center;gap:8px;font-size:13px;color:#374151;padding:.5rem .75rem;background:#f9fafb;border:1px solid #e5e7eb;border-radius:6px">
            ...
            <p style="font-size:12px;color:#9ca3af;text-align:center">Are you absolutely sure you want to proceed?</p>
        </div>
...
function renderRecord(issue, rec) {
    ...
    if (issue.type === 'duplicate') {
        ...
        actions = `
            <a href="${esc(rec.edit_url || '#')}" class="btn-sm warn">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5">
        ...
        actions = `
            <a href="${esc(rec.edit_url || '#')}" class="btn-sm warn">Review</a>
            <button class="btn-sm" onclick="ignoreRecord('${issue.id}','${rec.id}')">Ignore</button>`;
    }
...
function confirmDelete() {
    if (!DQ.pending) return;
    const { permitId, label, issueId, recId } = DQ.pending;
    closeDeleteModal();

    fetch(`/permits/${permitId}?source=data_quality`, {
        method: 'DELETE',
    ...
    .then(d => {
        ...
        // Find the issue and mark records no longer in fresh data as ignored
        const freshIssue = DQ.issues.find(i => i.id === issueId);
        const freshIds = freshIssue ? freshIssue.records.map(r => r.id) : [];

        // Any record in the DOM for this issue that's gone from fresh data → remove it
        const issueEl = document.getElementById('dqi-' + issueId);
        if (issueEl) {
            issueEl.querySelectorAll('.record').forEach(row => {
                const rid = row.id.replace('dqr-', '');
                if (!freshIds.includes(rid) && !DQ.ignored.has(rid)) {
                    DQ.ignored.add(rid);
                    DQ.resolvedCount++;
                    row.remove();
                }
    ...
    showToast('Deleted', `"${label}" was permanently removed.`, 'green');
})
    .catch(() => showToast('Error', 'Could not delete — please try again.', 'red'));
}
