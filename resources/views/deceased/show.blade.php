...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .renew-chip { display: inline-flex; align-items: center; gap: 5px; font-size: 12px; font-weight: 500; color: #374151; }
        .renew-count { display: inline-flex; align-items: center; justify-content: center; min-width: 22px; height: 22px; padding: 0 6px; border-radius: 11px; font-size: 11px; font-weight: 700; }
        ...
        html.dark .topbar-title, html.dark .topbar-sub { color: #e2e8f0 !important; }
        ...
        html.dark .btn-action { background: #252840 !important; border-color: #374151 !important; color: #cbd5e1 !important; }
        html.dark .btn-action:hover { background: #2d3148 !important; border-color: #6366f1 !important; color: #e2e8f0 !important; }
    ...
    <div class="content">
        ...
        <div class="panel">
            ...
            <div class="info-grid">
                ...
                {{-- Full Name spanning all 3 columns --}}
                <div class="info-item" style="grid-column: span 3; border-right: none;">
                    <div class="info-label">Full Name</div>
                    <div class="info-value" style="font-size:16px;font-weight:700;color:#1a2744">
                        {{ trim(
                            $deceased->last_name . ', ' .
                            $deceased->first_name .
                            ($deceased->middle_name ? ' ' . $deceased->middle_name : '') .
                            ($deceased->name_extension ? ' ' . $deceased->name_extension : '')
                        ) }}
                    </div>
                </div>
                ...
                <div class="info-item">
                    <div class="info-label">Next of Kin Contact</div>
                    <div class="info-value {{ !$deceased->phone_number ? 'empty' : '' }}">
                        {{ $deceased->phone_number ?? '—' }}
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">Name Extension</div>
                    <div class="info-value {{ !$deceased->name_extension ? 'empty' : '' }}">
                        {{ $deceased->name_extension ?? '—' }}
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">Name Number</div>
                    <div class="info-value {{ !$deceased->name_number ? 'empty' : '' }}">
                        {{ $deceased->name_number ?? '—' }}
                    </div>
                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="renew-chip">
                                    <span class="renew-count {{ $renewalCount > 0 ? 'active' : 'zero' }}">{{ $renewalCount }}</span>
                                    <span style="font-size:11px;color:#9ca3af">{{ $renewalCount === 1 ? 'renewal' : 'renewals' }}</span>
                                    <a href="{{ route('permits.show', $permit) }}" style="margin-left:.35rem;font-size:11px;color:#1a2744;text-decoration:none;opacity:.6" title="View permit">
        ...
        <form method="POST" action="{{ route('deceased.update', $deceased) }}">
            ...
            <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select name="name_extension" class="form-control">
                            ...
                            @foreach(['Jr.', 'Sr.', 'II', 'III', 'IV', 'V'] as $ext)
                                <option value="{{ $ext }}" {{ old('name_extension', $deceased->name_extension) === $ext ? 'selected' : '' }}>{{ $ext }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Name Number</label>
                        <input type="text" name="name_number" class="form-control" placeholder="e.g. 001" value="{{ old('name_number', $deceased->name_number) }}">
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="phone_number" class="form-control" placeholder="e.g. 09171234567" value="{{ old('phone_number', $deceased->phone_number) }}">
                </div>
