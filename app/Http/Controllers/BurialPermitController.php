...
class BurialPermitController extends Controller
{
    public function index()
    {
        ...
        return view('permits.index', compact('permits', 'recentPermits'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $latestPermit = BurialPermit::whereYear('created_at', now()->year)
            ->orderBy('id', 'desc')->first();

        $latest = 1;
        if ($latestPermit) {
            $parts  = explode('-', $latestPermit->permit_number);
            $latest = intval(end($parts)) + 1;
        }
    ...
    public function print(BurialPermit $permit)
    {
        ...
        $template->setValue('renewal_check',    $permit->is_renewal ? 'X' : '');
        $template->setValue('new_check',        !$permit->is_renewal ? 'X' : '');
    ...
    public function approve(BurialPermit $permit)
    {
        ...
        $permit->update(['status' => 'approved', 'processed_by' => Auth::id()]);
